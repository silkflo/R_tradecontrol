...
#'
#'
#'  path_terminal <- "C:/Program Files (x86)/AM MT4 - Terminal 3/MQL4/Files/"
#'  fileName <- "SystemControlMT"
#'  trading_system <- 9142202
#' 
#'
record_policy_mt_rl <- function(x, trading_system, path_terminal, fileName = "SystemControlMT"){
  requireNamespace("stringr", quietly = TRUE)
  
  # derive which terminal should be enabled (using path to sandbox) and using variable 'addition'
  is_T3 <- stringr::str_detect(path_terminal, "Terminal 3")
  is_T4 <- stringr::str_detect(path_terminal, "Terminal 4")
  if(is_T3 == TRUE) { addition <- 100 } else if(is_T4 == TRUE) { addition <- 200 } else { addition <- 0 }
